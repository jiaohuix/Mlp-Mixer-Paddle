...
#
#     Downloads checkpoint file into cache-dir like paddle.hub based loaders, but calls
#     a passed in custom load fun, or the `load_pretrained` model member fn.
#
#     If the object is already present in `model_dir`, it's deserialized and returned.
#     The default value of `model_dir` is ``<hub_dir>/checkpoints`` where
#     `hub_dir` is the directory returned by :func:`~paddle.hub.get_dir`.
#
#     Args:
...
#     conv_weight = conv_weight.float()  # Some weights are in paddle.half, ensure it's float for sum on CPU
...
def adapt_model_from_string(parent_module, model_string):
    ...
    for n, m in parent_module.named_modules():
        ...
        if isinstance(old_module, nn.BatchNorm2D):
            #???
            # new_bn = nn.BatchNorm2D( num_features=state_dict[n + '.weight'][0], epsilon=old_module.eps, momentum=old_module.momentum,affine=old_module.affine, track_running_stats=True)
            new_bn = nn.BatchNorm2D(num_features=state_dict[n + '.weight'][0], epsilon=old_module.eps, momentum=old_module.momentum, use_global_stats=True)
...
def update_default_cfg_and_kwargs(default_cfg, kwargs, kwargs_filter):
    ...
    if default_cfg.get('fixed_input_size', False):# 是否含该键，不存在则返回false
        # if fixed_input_size exists and is True, model takes an img_size arg that fixes its input size
        default_kwarg_names += ('img_size',)
    set_default_kwargs(kwargs, names=default_kwarg_names, default_cfg=default_cfg)# 用names和default_cfg更新kwargs
    # Filter keyword args for task specific model variants (some 'features only' models, etc.)
    filter_kwargs(kwargs, names=kwargs_filter)

# 用模型结构，结构配置，权重路由，加载完整的模型
def build_model_with_cfg(
    ...
    """
    pruned = kwargs.pop('pruned', False) # 是否有pruned键，有则弹出值；否则默认为false；决定是否剪枝√
    features = False
    ...
    # 是否加载预训练模型
    if pretrained:
        if pretrained_custom_load: #false
            pass
...
def named_apply(fn: Callable, module: nn.Layer, name='', depth_first=True, include_root=False) -> nn.Layer:
    if not depth_first and include_root: #非深度优先时递归初始化子模型
        fn(module=module, name=name)
    for child_name, child_module in module.named_children(): # #name为空，child_name就是子模型名，保持不变
        child_name = '.'.join((name, child_name)) if name else child_name
        named_apply(fn=fn, module=child_module, name=child_name, depth_first=depth_first, include_root=True) #遍历所有子模型，递归调用跳入第2个if，初始化子模型
    if depth_first and include_root: #深度优先时递归初始化子模型
        fn(module=module, name=name)
    return module


def named_modules(module: nn.Layer, name='', depth_first=True, include_root=False):
